...
const Register = () => {
  ...
  const navigate = useNavigate();  // Corrected this line by adding parentheses
  ...
  const registerUser = async (e) => {
    ...
    try {
      ...
      const newUser = response.data;  // Corrected how you access response data
      console.log(newUser);
      if(!newUser){
        setError('Could not register User. Please try again.')
      } else {
        navigate('/login');  // Correctly navigate to home
      }
    } catch (err) {
  ...
  return (
    <section className="register">
      <div className="container">
        ...
        <small>Already Have an Account? <Link to="/login" className='btn'>Sign In</Link></small>
      </div>
    </section>
  )
}
